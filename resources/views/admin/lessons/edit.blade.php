    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.lessons.update", [$lesson->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link_video') ? 'is-invalid' : '' }}" type="text" name="link_video" id="link_video" value="{{ old('link_video', $lesson->link_video) }}">
                @if($errors->has('link_video'))
                    <span class="text-danger">{{ $errors->first('link_video') }}</span>
                @endif
